...
namespace MinesweeperMVC.Services.Data
{
    ...
    public class ScoresDAO
    {
        ...
        public ScoresDAO()
        {
            MineLogger.getInstance().Info("ScoresDAO instantiated");
        ...
        {
            MineLogger.getInstance().Info("CheckHigh method invoked in ScoresDAO");
            try
            {
                ...
                while (dr.Read())
                {
                    ...
                    if (table == "dbo.highscorestime")
                    {
                        ...
                        if (oldTime > time)
                        {
                            MineLogger.getInstance().Info("User has broken a time highscore");
                            place = (int)dr["PLACE"];
                        }
                    }
                    else
                    {
                        ...
                        if (oldTurns > turns)
                        {
                            MineLogger.getInstance().Info("User has broken a turn highscore");
                            place = (int)dr["PLACE"];
                        }
                    }
                }
        ...
        {
            MineLogger.getInstance().Info("UpdateHigh method invoked in ScoresDAO");
            try
            {
                Int32 id = 0;
        ...
        {
            MineLogger.getInstance().Info("GetScores method invoked in ScoresDAO");
            try
            {
